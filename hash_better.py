...
class Hash:
    ...
    def Trash(self, filename):
        ...
        return interSections, cars

    def Counts(self, cars):
        carsFlat = self._MenuFlat(cars)
        counts = collections.Counter(carsFlat)
        counts = dict(counts)
        return counts

    def Ress(self, interSections, counts):
        ress = []
...
if __name__ == "__main__":
    ...
    for filename in filenames:
        interSections, cars = hash.Trash(filename)
        counts = hash.Counts(cars)
        ress = hash.Ress(interSections, counts)
